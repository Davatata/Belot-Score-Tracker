...
$('#team1_points').on('blur', function() {
    var t1points = 162 - parseInt($('#team1_points').val());
    if(t1points < 0 || t1points > 162){
    	badPointsInputed();
    }
    else{
    	$('#team2_points').val(t1points);
...
$('#team2_points').on('blur', function() {
    var t2points = 162 - parseInt($('#team2_points').val());
    if(t2points < 0 || t2points > 162){
    	badPointsInputed();
    }
    else{
    	$('#team1_points').val(t2points);
    	$('#team1_points').removeClass('is-invalid-input');
		$('#team2_points').removeClass('is-invalid-input');
    }
});
...
function badPointsInputed(){
	$('#team1_points').val(0);
	$('#team2_points').val(0);
	$('#team1_points').addClass('is-invalid-input');
	$('#team2_points').addClass('is-invalid-input');
}
	...
	if(!checkBet(contract_amt, attacker_chosen, suit_chosen, special_chosen, team1_points, team1_bonus, team2_points, team2_bonus)) return;
		...
		if(attacker_chosen == team1){
			if(team1_points + team1_bonus < contract_amt){
				...
				if(multiplier == 0){
					team1_total = team1_points + team1_bonus + contract_amt;
			...
			if(team2_points + team2_bonus < contract_amt){
				...
				if(multiplier == 0){
					...
					team2_total = team2_points + team2_bonus + contract_amt;
				}
				else{
					team1_total = 0;
					team2_total = contract_amt*multiplier + 162 + team1_bonus + team2_bonus;
...
function randomize(){
	...
	var special_random = Math.floor(Math.random() * 10) + 1;
	var special_chosen = '';
	if(special_random == 1) 	 special_chosen = "Coincher";
	else if(special_random == 2) special_chosen = "Sur-Coincher";
	else  						 special_chosen = ""; 
	$("#special_chosen").val(special_chosen);

	var bonuses = [0,0,0,0,0,0,0,0,20,20,20,20,20,50,100,150,200];
	var num = Math.floor(Math.random() * (bonuses.length-1)) + 1;
	var num2 = Math.floor(Math.random() * (bonuses.length-1)) + 1;
	var team1_points = rand_10(0, 170);
	$("#team1_points").val(team1_points);
	var team1_bonus = bonuses[num];
	$("#team1_bonus").val(team1_bonus);
	var team2_points = 162 - team1_points;
	$("#team2_points").val(team2_points);
	var team2_bonus = bonuses[num2];
